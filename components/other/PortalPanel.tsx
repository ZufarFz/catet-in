...
import { db, centralClient, centralUrl, centralAnonKey } from '../../supabase';
...
export const PortalPanel: React.FC<PortalPanelProps> = ({ onLogout, notify, confirm }) => {
  ...
  const [newSupabaseUrl, setNewSupabaseUrl] = useState(centralUrl || '');
  const [newSupabaseAnonKey, setNewSupabaseAnonKey] = useState(centralAnonKey || '');
  ...
  const handleCreateInstansi = async () => {
    ...
    try {
      ...
      setNewInstansiName('');
      setNewSupabaseUrl(centralUrl || '');
      setNewSupabaseAnonKey(centralAnonKey || '');
      setNewBackupUrl('');
  ...
  const activeUsers = users.filter(u => String(u.role).toLowerCase() !== 'pending' && String(u.status).toLowerCase() !== 'pending' && u.id !== 'portalmaster' && u.id !== 'super' + 'admin');
      ...
      <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between shadow-md shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Portal Control Panel</p>
          </div>
        </div>
        ...
        <div className="flex-1 bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col relative min-w-0">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">
              {activeTab === 'pending' ? 'Daftar Pengajuan Akun Baru' : activeTab === 'active' ? 'Daftar Pengguna Aktif' : activeTab === 'instansi' ? 'Kelola Database Instansi Cabang' : 'Aturan Keamanan & Setup Supabase (Multi-DB)'}
            </h3>
            {activeTab === 'instansi' && (
              <button
                onClick={() => {
                  ...
                  >
                    <div className="space-y-1.5 min-w-0 flex-1">
                      ...
                      <div className="space-y-1 text-[8.5px] font-bold text-slate-500 tracking-tight">
                        <div className="flex items-center gap-1.5 text-blue-600 font-black uppercase tracking-widest font-mono">
                          <Database size={11} className="text-blue-500" />
                          <span>Database Terpusat (Patented 1-DB System)</span>
                        </div>
            ...
            <div className="p-5 sm:p-6 space-y-4">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <option value="PortalMaster">PORTAL MASTER</option>
                </select>
              </div>

              {/* Instansi Select from database config */}
              <div className="space-y-1">
